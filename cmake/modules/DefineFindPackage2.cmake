...
function(find_package2 pkgname)
    ...
    set(_find_anyproject_args)
    if(arg_REQUIRED)
        list(APPEND _find_anyproject_args REQUIRED)
    elseif(arg_QUIET)
        list(APPEND _find_anyproject_args QUIET)
    endif()

    find_anyproject(${pkgname} ${_find_anyproject_args})
    if(DEFINED ${key}_FOUND)
        if(NOT ${${key}_FOUND})
            unset(_find_anyproject_found)
        else()
            set(_find_anyproject_found TRUE)
        endif()
    endif()
    if(_find_anyproject_found)
        set_property(GLOBAL APPEND PROPERTY PACKAGES_FOUND "${pkgname}")
        if(arg_OUT_DEPENDENCY)
            if(arg_REQUIRED)
                set(_find_anyproject_required REQUIRED)
            else()
                set(_find_anyproject_required "")
            endif()
            set(${arg_OUT_DEPENDENCY}
                "find_anyproject(${pkgname} ${_find_anyproject_required})\n"
                PARENT_SCOPE)
        endif()
        if(DEFINED ${key}_INCLUDE_DIRS)
            set(${key}_INCLUDE_DIRS "${${key}_INCLUDE_DIRS}" PARENT_SCOPE)
        endif()
        if(DEFINED ${key}_INCLUDE_DIR)
            set(${key}_INCLUDE_DIR "${${key}_INCLUDE_DIR}" PARENT_SCOPE)
        endif()
        if(DEFINED ${key}_LIBRARIES)
            set(${key}_LIBRARIES "${${key}_LIBRARIES}" PARENT_SCOPE)
        endif()
        if(DEFINED ${key}_LIBRARY)
            set(${key}_LIBRARY "${${key}_LIBRARY}" PARENT_SCOPE)
        endif()
        # Keep the function contract: expose an imported target even when
        # the dependency comes from find_anyproject().
        set(_find_package2_include_dirs)
        foreach(_inc_var IN ITEMS
                ${key}_INCLUDE_DIRS ${key}_INCLUDE_DIR
                ${pkgname}_INCLUDE_DIRS ${pkgname}_INCLUDE_DIR)
            if(DEFINED ${_inc_var} AND NOT "${${_inc_var}}" STREQUAL "")
                set(_find_package2_include_dirs "${${_inc_var}}")
                break()
            endif()
        endforeach()

        set(_find_package2_link_libraries)
        foreach(_lib_var IN ITEMS
                ${key}_LIBRARIES ${key}_LIBRARY
                ${pkgname}_LIBRARIES ${pkgname}_LIBRARY)
            if(DEFINED ${_lib_var} AND NOT "${${_lib_var}}" STREQUAL "")
                set(_find_package2_link_libraries "${${_lib_var}}")
                break()
            endif()
        endforeach()

        set(_find_package2_targets ${pkgname}::${pkgname} ${key}::${pkgname})
        list(REMOVE_DUPLICATES _find_package2_targets)
        foreach(_target_name IN LISTS _find_package2_targets)
            if(NOT TARGET ${_target_name})
                add_library(${_target_name} INTERFACE IMPORTED)
            endif()
            set(_target_link_libraries "${_find_package2_link_libraries}")
            # Avoid self-reference when a find module reports a not-yet-created target name.
            list(REMOVE_ITEM _target_link_libraries ${_target_name})
            if(_target_link_libraries)
                set_target_properties(${_target_name} PROPERTIES
                                      INTERFACE_LINK_LIBRARIES "${_target_link_libraries}")
            endif()
            if(_find_package2_include_dirs)
                set_target_properties(${_target_name} PROPERTIES
                                      INTERFACE_INCLUDE_DIRECTORIES "${_find_package2_include_dirs}")
            endif()
        endforeach()
        set(${key}_FOUND TRUE PARENT_SCOPE)
        return()
    endif()
